...
unsigned long onTime = 0;  // Almacena el tiempo en el que la key cambió a "ON"
bool isOn = false;  // Indica si la key está actualmente en "ON"
...
void servo1Callback(char *data, uint16_t len) {
  ...
  if (message == "ON") {
    onTime = millis();  // Guarda el tiempo actual
    isOn = true;  // Cambia el estado a "ON"
    ...
    isOn = false;  // Cambia el estado a "OFF"
  }
}
...
void loop() {
  ...
  // Si la key está en "ON" y han pasado 5 segundos desde que cambió a "ON"
  if (isOn && millis() - onTime >= 5000) {
